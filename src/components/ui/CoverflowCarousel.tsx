// Coverflow Carousel — Originkit (adapted for React / Vite)
// Source: https://originkit.io — component defaults preserved.
...
import { playScrollSound, playHoverSound } from "../../lib/sound"
...
function Card({
    ...
    const boxShadow = useTransform(pos, (p: number) =>
        Math.abs(relOf(index, p, count)) < 0.5
            ? "0 24px 70px rgba(0,0,0,0.75), inset 0 0 0 1px rgba(255,255,255,0.06)"
...
const COMPONENT_DEFAULTS: Required<Omit<Props, "style" | "onIndexChange">> = {
...
export default function CoverflowCarousel(props: Props) {
    ...
    const moveDur =
        typeof transitionProp?.duration === "number" ? transitionProp.duration : 0.35
    const dwell =
    ...
    const reportIndex = useCallback((rawTarget: number) => {
        if (!onIndexChange) return
        let idx = ((rawTarget % count) + count) % count
    ...
    const goTo = useCallback(
        ...
        [ensureRunning, count, reportIndex]
    )
